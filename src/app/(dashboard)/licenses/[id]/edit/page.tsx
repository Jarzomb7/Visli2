...
export default function EditLicensePage() {
  ...
  useEffect(() => {
    fetch(`/api/licenses/${id}`)
      ...
      .then((d) => {
        if (d.license) {
          setLicenseKey(d.license.key);
          setDomain(d.license.domain);
          setPlan(d.license.plan);
          setStatus(d.license.status);
          setExpiresAt(d.license.expiresAt.split("T")[0]);
        }
      })
      .catch(console.error)
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/licenses/${id}`, {
        ...
        body: JSON.stringify({ domain, plan, status, expiresAt: new Date(expiresAt).toISOString() }),
      });
