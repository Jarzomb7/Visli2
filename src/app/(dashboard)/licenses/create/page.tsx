    ...
    fetch("/api/products")
      ...
      .then((d) => {
        if (d.products) {
          ...
          if (d.products.length > 0) setProductId(d.products[0].id);
        }
      })
      .catch(console.error);
  }, []);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch("/api/licenses", {
        ...
        body: JSON.stringify({ domain, plan, duration, productId }),
      });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {products.length === 0 && (
                <p className="col-span-2 text-xs text-white/25">Loading products...</p>
              )}
            </div>
          </div>
