...
export async function GET() {
  try {
    ...
    return NextResponse.json({ subscriptions, plans: BILLING_PLANS });
  } catch (err) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    let body: { subscriptionId?: number; plan?: string };
    try { body = await request.json(); } catch { return NextResponse.json({ error: "Invalid body" }, { status: 400 }); }

    if (!body.subscriptionId || !body.plan) {
      return NextResponse.json({ error: "subscriptionId and plan are required" }, { status: 400 });
    }
    ...
    const nextPlan = getBillingPlan(body.plan);
    if (!nextPlan) return NextResponse.json({ error: "Invalid plan" }, { status: 400 });

    const priceId = await getSetting(nextPlan.settingKey, "");
    ...
    const product = await prisma.product.upsert({
      where: { code: nextPlan.productCode },
      update: {
        name: nextPlan.name,
    ...
    await prisma.subscription.update({
      ...
      data: {
        plan: nextPlan.id,
        productCode: nextPlan.productCode,
